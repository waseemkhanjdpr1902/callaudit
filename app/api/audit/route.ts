...
9. Answer speed: Estimate when the agent first answers from the beginning of the recording. Ignore IVR, ringing, music and customer speech. Grade 0-5 seconds Excellent, 6-10 Acceptable, 11-15 Needs improvement and above 15 Breach. If the recording is clipped, starts after connection, or the speaker cannot be identified, return Not assessable. This is an audio-based estimate, not a telephony SLA timestamp.
10. Feedback quality: Produce short, neutral observations tied to audible evidence and a specific improvement action for each material gap. Include strengths as positive observations. Repeated random "hello", avoidable interruption, long dead air, poor hold handling, wrong client code and cross-customer disclosure must be called out when evidenced. Never infer a data breach merely because two identifiers sound different; require clear contextual evidence.
...
function auditPrompt(criteria: Criterion[], transcript?: string) {
  return `${SYSTEM_PROMPT}\n\nScorecard: ${JSON.stringify(criteria)}${transcript ? `\n\nTimestamped call transcript:\n${transcript}` : "\n\nTranscribe and audit the attached call."}\n\nBefore returning the result, silently build an evidence ledger and cross-check that each finding agrees with its evidence. Return JSON only with this exact structure: {"overallScore": number 0-100, "language": string, "summary": string, "disposition": "Pass"|"Needs coaching"|"Critical", "answerTiming": {"seconds": number|null, "rating": "Excellent"|"Acceptable"|"Needs improvement"|"Breach"|"Not assessable", "basis": string}, "sections": [{"name": string, "score": number 0-100, "weight": number, "finding": string, "evidence": string}], "feedback": [{"observation": string, "action": string, "severity": "Positive"|"Coaching"|"Critical", "evidence": string}], "criticalFailures": string[], "strengths": string[], "coaching": string[], "transcriptExcerpt": string}. Include every supplied scorecard section exactly once, using its supplied name and weight. Create concise feedback rows for material findings; do not duplicate the same issue. Calculate overallScore as the weighted average of section scores. Mark Critical only for a material regulatory, privacy, fraud, abuse, unauthorized-trading, mis-selling or mandatory-compliance failure.`;
}
...
async function fetchWithRetry(
  ...
  for (let attempt = 1; attempt <= attempts; attempt++) {
    try {
      ...
      console.warn(
        `[${label}] network attempt ${attempt} failed; retrying`,
        error,
      );
...
async function auditWithGroq(
  ...
  if (!transcriptionResponse.ok)
    throw new Error(
      await providerError(transcriptionResponse, "Groq transcription"),
    );
  const transcription = await transcriptionResponse.json();
  if (!transcription.text) throw new Error("Groq returned an empty transcript");
  const timestampedTranscript = Array.isArray(transcription.segments)
    ? transcription.segments
        .map(
          (segment: { start?: number; end?: number; text?: string }) =>
            `[${Number(segment.start || 0).toFixed(1)}-${Number(segment.end || 0).toFixed(1)}s] ${String(segment.text || "").trim()}`,
        )
        .join("\n")
    : transcription.text;
  ...
  for (const model of models) {
    const auditResponse = await fetchWithRetry(
      "https://api.groq.com/openai/v1/chat/completions",
      {
        ...
        body: JSON.stringify({
          ...
          messages: [
            ...
            {
              role: "user",
              content: auditPrompt(criteria, timestampedTranscript),
            },
          ],
        }),
      },
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (
      ...
      let blobUrl: URL;
      try {
        blobUrl = new URL(String(body.audioUrl || ""));
      } catch {
        return NextResponse.json(
          { error: "A valid temporary audio upload is required." },
          { status: 400 },
        );
      }
      if (
      ...
      if (source.ok) {
        ...
        const privateSource = await get(blobUrl.href, {
          access: "private",
          useCache: false,
        });
        if (!privateSource || privateSource.statusCode !== 200)
          throw new Error(
            `Temporary audio download failed with ${source.status}`,
          );
        if (privateSource.blob.size > MAX_AUDIO_BYTES)
          return NextResponse.json(
            { error: "Audio files must be 25 MB or smaller." },
            { status: 413 },
          );
      ...
      audio = new File([bytes], String(body.fileName || "recording.mp3"), {
        type: String(body.fileType || sourceType || "audio/mpeg"),
      });
    ...
    if (
      ...
      !/\.(mp3|wav|m4a|ogg|aac|flac)$/i.test(audio.name)
    )
      return NextResponse.json(
    ...
    try {
      criteria =
        typeof criteriaValue === "string"
          ...
          : (criteriaValue as Criterion[]);
    } catch {
      return NextResponse.json(
        { error: "The audit checklist is invalid." },
        { status: 400 },
      );
    }
    if (!Array.isArray(criteria) || !criteria.length || criteria.length > 30)
      return NextResponse.json(
        { error: "Add between 1 and 30 audit parameters." },
        { status: 400 },
      );
    criteria = criteria
      .map((item) => ({
    ...
    if (
      Math.round(criteria.reduce((sum, item) => sum + item.weight, 0)) !== 100
    )
      return NextResponse.json(
        { error: "Audit parameter weights must total 100%." },
        { status: 400 },
      );
    ...
    if (!geminiKey && !groqKey)
      return NextResponse.json(
        {
          error:
            "AI auditing is not configured. Add GEMINI_API_KEY or GROQ_API_KEY in Vercel.",
        },
        { status: 503 },
      );
    ...
    if (geminiKey) {
      try {
        result = await auditWithGemini(audio, criteria, geminiKey);
        provider = "gemini";
      } catch (error) {
        const message =
          error instanceof Error ? error.message : "Gemini failed";
        providerErrors.push(message);
        console.warn("Gemini audit failed; trying Groq fallback.", error);
      }
    }
    if (!result && groqKey) {
      try {
        result = await auditWithGroq(audio, criteria, groqKey);
        provider = "groq";
      } catch (error) {
    ...
    if (!result)
      return NextResponse.json(
        {
          error: "AI providers could not process this recording.",
          details:
            providerErrors.join(" · ") ||
    ...
    return NextResponse.json(
      { result: { ...result, fileName: audio.name }, provider },
    ...
    return NextResponse.json(
      {
        error:
          "This recording could not be audited. Please verify the file and try again.",
      },
      { status: 500 },
    );
  } finally {
    if (temporaryBlobUrl) {
      try {
        ...
        console.warn("[audit] temporary blob cleanup failed", error);
      }
    }
  }
}
